...
################ 해당 위치에서 기존에 있던 new_datasets폴더 제거
# 한번 실행될때마다 한번씩 삭제 ############################
# viton/detectron2/datasets/new_datasets 안의 파일 삭제
# 단, 폴더가 처음에 존재하지 않을 수도 있으니 직접 만들어주자..?
# (나중에 고칠 것)
def init_del(pth='viton/detectron2/datasets/new_datasets'):
    dir_path=Path(pth)
    dir_path.rmdir()

    # 원래 기능하던 민석님 코드(ipynb용)
...
## 필요한 파일 및 디렉토리 생성. 
# make_inference_dataset_decompo.py 내부적으로 파일, 디렉토리 생성하므로 확인 필요
# 단, 이 작업은 이 파일과 분리되어 실행될 수 있기 때문에
# ipynb 파일만으로 동작할 것임. 그러니 ipynb파일로 실행
...
def init_setup(pth='viton/detectron2'):
    cfg = get_cfg()
    cfg.merge_from_file(os.path.join(pth, "configs/COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_3x.yaml"))
...
#### 초기화 세팅에 필요한 부분 한번만, 그대로 놔둬도 동작에 문제 없을 듯 ######
detector = Detector()
#### 초기화 세팅에 필요한 부분 한번만, 그대로 놔둬도 동작에 문제 없을 듯 ######
...
#### 초기화 세팅에 필요한 부분 한번만 ######
###### 이미지 directory path, os.getcwd() 경로 바꾸기 조심
# 경로를 web_main.py기준으로 바꿈
all_img_dir_path = os.getcwd() + 'viton/detectron2/datasets/new_datasets'
...
def api_one(origin_dir, test_img_dir, origin_img_name, predictor, pth='viton/detectron2'): #이미지 정보를 받고나서 resized된 이미지들 , annot 처리
    ...
    #edge 만들기
    # make_inference_dataset_decompo.py를 실행시킬 때 u2net_human_seg_TDWV이 만들어진다.
    
    from U_2_Net import u2net_human_seg_TDWV as human_seg
    ...
    file_path=Path(os.path.join(pth, 'datasets/new_datasets/test_img/.ipynb_checkpoints'))
    file_path.unlink()
    
    tmp_pth=os.path.join(pth, 'Self-Correction-Human-Parsing-for-ACGPN/simple_extractor2.py')
    exec(f'python3 f{tmp_pth} --input-dir ../datasets/new_datasets/test_img_nobg/ --output-dir ../datasets/new_datasets/test_label/ --model-restore ./checkpoints/exp-schp-201908261155-lip.pth')
...
def api_two(origin_dir, test_clothes_dir, origin_img_name, pth='viton/detectron2'): #옷 이미지 , 옷 seg 이미지, 이미지 이름
    #%cd datasets/new_datasets/test_img
    ...
    tmp_pth=os.path.join(pth, 'clothes_seg.py')
    exec(f'python {tmp_pth}')
