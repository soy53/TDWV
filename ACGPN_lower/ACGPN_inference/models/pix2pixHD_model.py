...
class Pix2PixHDModel(BaseModel):
    ...
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose,grid,mask_fore):
        ...
        arm1_mask = torch.FloatTensor((label.cpu().numpy() == 9).astype(np.float)).cuda()
        arm2_mask = torch.FloatTensor((label.cpu().numpy() == 10).astype(np.float)).cuda()
        ...
        new_arm1_mask = torch.FloatTensor((armlabel_map.cpu().numpy() == 9).astype(np.float)).cuda()
        new_arm2_mask = torch.FloatTensor((armlabel_map.cpu().numpy() == 10).astype(np.float)).cuda()
        ...
        armlabel_map = armlabel_map * (1 - arm1_full) + arm1_full * 9
        armlabel_map = armlabel_map * (1 - arm2_full) + arm2_full * 10
